    ...
    <template v-else>
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p class="text-secondary-500">{{ $t('bestellungen.edit', 'Bestellung bearbeiten') }}</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-6">
          ...
          <div class="card p-6">
            ...
            <div class="space-y-4">
              <div>
                <label class="block text-sm text-secondary-600 mb-1">{{ $t('bestellungen.status', 'Status') }}</label>
                <select v-model="bestellung.Bestellstatus" class="input">
...
const breadcrumbs = computed(() => [
  ...
  { label: bestellung.value?.Bestellnummer || t('bestellungen.edit', 'Bearbeiten') }
])
...
async function fetchBestellung() {
  try {
    ...
    bestellung.value = {
      ...response,
      Lieferdatum: response.Lieferdatum ? new Date(response.Lieferdatum).toISOString().split('T')[0] : ''
    }
    selectedKunde.value = response.Kunden

    // Map positions
    positionen.value = (response.Bestellpositionen || []).map((pos: any) => ({
      Produkt: pos.Produkte,
      ProduktID: pos.ProduktID,
      Menge: Number(pos.Menge),
      EinzelpreisNetto: Number(pos.EinzelpreisNetto),
      GesamtpreisNetto: Number(pos.GesamtpreisNettoPosition),
...
async function saveBestellung() {
  ...
  try {
    ...
    toast.success(t('bestellungen.saveSuccess', 'Bestellung erfolgreich gespeichert'))
    navigateTo('/bestellungen')
  } catch (err: any) {
    toast.error(t('bestellungen.errorSaving', 'Fehler beim Speichern der Bestellung'))
    console.error('Save Bestellung Error:', err)
  } finally {
    saving.value = false
  }
}
...
onMounted(async () => {
  await Promise.all([fetchBestellung(), fetchStandorte()])
})
</script>
