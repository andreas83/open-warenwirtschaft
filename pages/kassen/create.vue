<template>
  <div class="container mx-auto py-6">
    <h2 class="text-3xl font-bold mb-6 text-gray-800">{{ $t('kassen.newCashRegister') }}</h2>

    <form @submit.prevent="createKasse" class="bg-white rounded-lg shadow-md p-6">
      ...
      <div class="flex gap-3 mt-6">
        <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition duration-200">{{ $t('kassen.create') }}</button>
...
async function createKasse() {
  try {
    // Set AktuellerBestand to Anfangsbestand on creation
    kasse.value.AktuellerBestand = kasse.value.Anfangsbestand

    await $fetch('/api/kassen', {
      method: 'POST',
      body: kasse.value
    })
    router.push('/kassen')
  } catch (err) {
    alert('Fehler beim Erstellen der Kasse: ' + err.message)
    console.error('Error creating kasse:', err)
  }
}
