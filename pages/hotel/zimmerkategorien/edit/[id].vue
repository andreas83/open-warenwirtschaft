  ...
  <div class="p-6">
    <div class="max-w-3xl mx-auto">
      ...
      <div v-else class="bg-white rounded-lg shadow-md p-6">
        <h1 class="text-2xl font-bold text-gray-800 mb-2">{{ t('hotel.categories.editCategory', 'Kategorie bearbeiten') }}</h1>
        <p class="text-gray-600 mb-6">{{ t('hotel.categories.editSubtitle', 'Bearbeiten Sie die Kategoriedaten') }}</p>
...
async function loadCategory() {
  loading.value = true
  try {
    const category = await $fetch(`/api/hotel/zimmerkategorien?id=${categoryId.value}`)
    form.value = {
      name: category.Name || '',
      beschreibung: category.Beschreibung || '',
      grundpreis: category.Grundpreis ? parseFloat(category.Grundpreis) : null,
      maxPersonen: category.MaxPersonen || 2,
      groesse: category.Groesse ? parseFloat(category.Groesse) : null,
      bildURL: category.BildURL || '',
      ausstattung: category.Ausstattung ? JSON.parse(category.Ausstattung) : [],
      istAktiv: category.IstAktiv !== false
    }
  } catch (error) {
